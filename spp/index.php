            ...
            <div class="table-responsive">
              <table class="table table-bordered">
                ...
                <tbody id="tableBody">
                  ...
                  $countQuery = "SELECT COUNT(*) AS total FROM SPP WHERE tahun LIKE '%$cari%' OR nominal LIKE '%$cari%'";
                  $totalData = $database->ambil_data($countQuery);
                  $totalData = $totalData[0]['total'];

                  $nextDisabled = ($totalData <= $maju) ? 'disabled' : '';

                  $query = "SELECT * FROM SPP WHERE tahun LIKE '%$cari%' OR nominal LIKE '%$cari%' LIMIT $limit OFFSET $offset";
